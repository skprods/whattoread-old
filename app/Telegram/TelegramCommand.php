...
use SKprods\Telegram\Core\Command;
...
abstract class TelegramCommand extends Command
{
    ...
    protected function handleInvalidArguments(Collection $required, Collection $arguments)
    {
        ...
        $e = new Exception("Не переданы обязательные аргументы! Убедитесь, что каждый присутствует: $requiredArgs");
    ...
    public function replyWithMessage(array $sendMessageParams): mixed
    {
        $this->addResponse($sendMessageParams);

        try {
            return parent::replyWithMessage($sendMessageParams);
        } catch (\Exception $exception) {
            $this->logError($sendMessageParams, $exception);
